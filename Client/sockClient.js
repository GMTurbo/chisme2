...
var onData = function(data){
  socket.emit('sendData', { type : 'data' , chunk: data} );
};

var onEnd = function(){
  socket.emit('sendDataDone', { type : 'data' , chunk: null} );
};
...
var file = {name: '', stream: null, size: 0 };
	...
	switch(cmd){
		...
		case 'send':
		    ...
		    fs.exists(file, function(exists){
		      if(exists){
		        console.log('sending file');
		        socket.emit('sendStart', {filename: path.basename(file), size: fs.statSync(file)["size"], to: to, from: nick});
		        fs.createReadStream(file).pipe(tr);
		        
		        // ss(socket).emit('sendfile', stream,
		        //   {filename: 'name.txt', to: to, from: nick});
		      }
		    })
		  break;
	}
